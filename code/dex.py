...
class DEX:
    def __init__(self, num_tokens):
        self.initial_tokens = [0]*num_tokens
    ...
    def get_relative_price(self, idx1, idx2):  # price of any token in terms of token1
        if idx1>=len(self.tokens) or idx2>=len(self.tokens):
            raise IndexError("Index out of range.")
        if self.tokens[idx1] == 0:
            return math.inf
        if idx1==0:                                     #price of token0 in terms of token1
            return self.tokens[idx2]/self.tokens[idx1]
        return 1.0                                      #price of token1 in terms of token1
    
    def get_swap(self, idx1, idx2, amt):    # swap amt of idx1 for idx2
        if idx1>=len(self.tokens) or idx2>=len(self.tokens):
            raise IndexError("Index out of range.")
        if amt<0:
